...
<body>
    <div class="container">
        <h1 class="text-center">Listas Denegadas @role('cotrrsa hombres') COTRRSA Hombres @endrole @role('cotrrsa mujeres') COTRRSA Mujeres @endrole</h1>
        
        <div class="row ">

            @role('administracion')
            <div class="col">
                <form id="formDeleteAll" action="{{ route('denyFiles.clearFiles') }}" method="POST">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="btn btn-danger " id="btnDeleteDenyFiles">Limpiar Tabla</button>
                </form>
            </div>
            @endrole

            <div class="col">
                <button class="btn btn-primary float-end mx-5" data-bs-toggle="modal" data-bs-target="#filterDenyModal">Filtrar</button>
            </div>
        ...
        <table class="table text-center" id="productTable">
            <thead>
                <tr>
                    ...
                    <th scope="col">Fecha y Hora de Archivo Denegado
            ...
            <tbody>
                ...
                <tr class="category-{{ $pdfDatasH->category }}">
                    ...
                    <td class="col-sm-1">
                        <a href="{{ route('pdf.showFileDeny', $pdfDatasH->id) }}" target="_blank" class="btn btn-primary">Ver Archivo</a>
                    </td>
                </tr>
                @endforeach
                @endrole
                ...
                <tr class="category-{{ $pdfDatasM->category }}">
                    ...
                    <td class="col-sm-1">
                        <a href="{{ route('pdf.showFileDeny', $pdfDatasM->id) }}" target="_blank" class="btn btn-primary">Ver Archivo</a>
                    </td>
                </tr>
                @endforeach
                @endrole
