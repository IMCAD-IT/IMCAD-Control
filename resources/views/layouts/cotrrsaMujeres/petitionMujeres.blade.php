...
<head>
    ...
    <title>Crear Lista Mujeres</title>
</head>
...
<body>
    ...
    <div class="container">
        ...
        <table class="table text-center">
            ...
            <tbody>
                @foreach($petitionsM as $petitionM)
                <tr>
                    <td>{{ $petitionM->name }}</td>
                    <td>{{ $petitionM->quantity }}</td>
                    <td>{{ $petitionM->partsAvailable }}</td>
                    <td>{{ $petitionM->supplier }}</td>
                    <td>{{ $petitionM->observations }}</td>
                    <td>{{ $petitionM->category }}</td>
                    <td>{{$petitionM->created_at}}</td>
                    <td>
                        <form action="{{ route('petitionM.destroy',  $petitionM->id) }}" method="post">
                            @csrf
                            @method('delete')
        ...
        <form id="formDeleteAll" action="{{ route('petitionMujeres.clearProducts') }}" method="POST">
            @csrf
            @method('DELETE')
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="{{ route('savePetitionMujeres.saveProduct') }}" method="POST">
