...
class InventarioMujeresController extends Controller
{
    ...
    public function __construct()
    {
        ...
        $this->inventario = 'cotrrsa_mujeres_inventario';
    }

    public function cotrrsaMujeresInsert(Request $request)
    {
        ...
        try {
            CotrrsaMujeresInventario::create([
    ...
    {
        $cotrrsaMProduct = CotrrsaMujeresInventario::findOrFail($id);
        $imagePath = $cotrrsaMProduct->image;
        // Crear el registro de acción
        $nombreProducto = $cotrrsaMProduct->name;
        $total = '-' . $cotrrsaMProduct->quantity;
        $action = "Se ELIMINO el producto: " . $nombreProducto;
        $userId = Auth::id();
    ...
    public function actualizarCantidad(Request $request, $id)
    {
        ...
        $currentQuantity = $cotrrsaMProduct->quantity;
        ...
        if ($nuevaCantidad <= 0) {
            $producto = CotrrsaMujeresInventario::findOrFail($id);
            ...
            // Crear el registro de acción
            $action = "Se ACTUALIZÓ la cantidad del producto: " . $nombreProducto . " de: " . $cantidadProducto . " a: " . $nuevaCantidad;
            $userId = Auth::id();
            $inventario = $this->inventario;
            ...
            ActionLog::create([
                'user_id' => $userId,
                'action' => $action,
                'quantity' => $nuevaCantidad - $cantidadProducto,
                'inventario' => $inventario,
                'details' => $request->input('message-food'),
                'details_id' => $details_id,
            ]);
            ...
            $cotrrsaMProduct->quantity = $nuevaCantidad;
            $cotrrsaMProduct->save();
